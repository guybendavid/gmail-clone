...
const MessageTypeToText = {
  ...
  CONTAINER_STYLE_NAMING:
    "Parent/container elements should use 'containerStyle' for objects or 'getContainerStyle' for functions.",
  CONTAINER_STYLE_RESERVED:
    "The names 'containerStyle' and 'getContainerStyle' are reserved for parent/container elements only.",
  ...
  PREFER_DIRECT_FUNCTION_REFERENCE:
    "Use direct function reference instead of arrow function when no arguments are passed.",
  ...
  NO_GET_PREFIX_FOR_VOID_FUNCTIONS:
    "Void functions (functions that don't return values) should not start with 'get' prefix.",
...
export const customRuleMap: Record<string, Rule.RuleModule> = {
      ...
      return {
        ...
        JSXAttribute: (node: any) => {
          ...
          if (!isOutermost && cssStyles.has(classNameValue)) {
            context.report({
              node:
                node.value.expression.type === "CallExpression" ? node.value.expression.callee : node.value.expression,
            ...
            context.report({
              ...
              fix: (fixer: any) => {
                ...
                const insertionTarget =
                  componentExportNodes.length > 0 ? componentExportNodes[0] : componentDeclarationNodes[0];
  ...
  "blank-line-after-setters": {
    ...
    create: (context: Rule.RuleContext) => ({
      ...
      ExpressionStatement: (node: any) => {
        ...
        const functionName =
          callExpr.callee?.type === "Identifier" && callExpr.callee.name ? callExpr.callee.name : "function call";
  ...
  "prefer-direct-function-reference": {
    meta: {
      ...
      docs: {
        description:
          "Prefer direct function references over arrow functions in JSX event handlers when no arguments are passed",
    ...
    create: (context: Rule.RuleContext) => {
      const checkFunction = (node: AnyNode) => {
        ...
        const isNextJsRouteHandler =
          ...
          parent.id?.name &&
          /^(GET|POST|PUT|DELETE|PATCH)$/.test(parent.id.name);
